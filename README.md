...
Tags instead of folders. Full-text search. Multi-account with OAuth (Gmail, Microsoft 365), JMAP, and password auth. Mail syncs to a local SQLite store, encrypted at rest by default with a searchable full-text index. Configurable keybindings, filter rules, and HTML signatures. All in a native GUI backed by a Go CLI.
